...
def find_closest(location, centroids):
    ...
    [2.0, 3.0]
    """
    index, min_distance = centroids[0], distance(location, centroids[0])
    for a in centroids:
        if distance(location, a)<min_distance:
            min_distance = distance(location, a)
            index = a
    return index
...
def group_by_centroid(restaurants, centroids):
    ...
    >>> groups
    [[['A', [-10, 2], [], 2, [['A', 4]]], ['B', [-9, 1], [], 3, [['B', 5], ['B', 3.5]]]], [['C', [4, 2], [], 1, [['C', 5]]], ['D', [-2, 6], [], 4, [['D', 2]]], ['E', [4, 2], [], 3.5, [['E', 2.5], ['E', 3]]]]]
    """
    pairs = []
    restaurant_locations = [[restaurant_location(r), r] for r in restaurants]
    for e in restaurant_locations:
        x = find_closest(e[0], centroids)
        pairs.append([x, e[1]])
...
def find_centroid(cluster):
    ...
    lats = [a[0] for a in locations]
    longs = [a[1] for a in locations]
...
def k_means(restaurants, k, max_updates=100):
    ...
    while old_centroids != centroids and n < max_updates:
        ...
        centroids = [find_centroid(b) for b in clusters]
        n += 1
    return centroids
...
def find_predictor(user, restaurants, feature_fn):
    ...
    meanxs, meanxy = mean(xs), mean(ys)

    sxx = sum([(x-meanxs)**2 for x in xs])
    syy = sum([(y-meanxy)**2 for y in ys])
    sxy = sum([(x-meanxs)*(y-meanxy) for x, y in xys])
...
def rate_all(user, restaurants, feature_fns):
    ...
    reviewed = user_reviewed_restaurants(user, restaurants)
    x = {restaurant_name(r): user_rating(user, restaurant_name(r)) for r in reviewed}
    x.update({restaurant_name(r): predictor(r) for r in restaurants if r not in reviewed})
    return x
...
def search(query, restaurants):
    ...
    restaurants -- A sequence of restaurants
    """
    categories_and_restaurants = [[restaurant_categories(r), r] for r in restaurants]
    filtered = []
    for e in categories_and_restaurants:
        if query in e[0]:
            filtered.append(e[1])
    return filtered
